...
export function useOverlay(key) {
  ...
  const patch = useCallback(
    (id, partial) => {
      ...
      const next = { ...current, [id]: { ...(current[id] || {}), ...partial } };
      writeJSON(key, next);
      setOverlay(next);
    },
    [key]
...
export function useCustom() {
  const [custom, setCustom] = useState(() =>
    readJSON(KEYS.custom, { cases: [], clauses: [] })
  );

  useEffect(() => {
    const reload = (e) => {
      if (!e.detail || e.detail.key === KEYS.custom)
        setCustom(readJSON(KEYS.custom, { cases: [], clauses: [] }));
    };
    window.addEventListener("frp:changed", reload);
  ...
  const addCase = useCallback((entry) => {
    ...
    const next = { ...current, cases: [...(current.cases || []), entry] };
  ...
  const addClause = useCallback((entry) => {
    const current = readJSON(KEYS.custom, { cases: [], clauses: [] });
    const next = { ...current, clauses: [...(current.clauses || []), entry] };
    writeJSON(KEYS.custom, next);
    setCustom(next);
  }, []);
  ...
  const remove = useCallback((kind, id) => {
    ...
    const next = {
      ...current,
      [listKey]: (current[listKey] || []).filter((x) => x.id !== id),
    };
    writeJSON(KEYS.custom, next);
    setCustom(next);
  }, []);
